...
pub fn first_resistance_support_price(df: &DataFrame, prices: &Vec<StockDailyPrice>) -> (BigDecimal, BigDecimal) {
    ...
    if !resistance_indexes.is_empty() {
        min_resistance_price = BigDecimal::from_f32(*ma_prices.get(resistance_indexes[0]).unwrap()).unwrap();
        for i in resistance_indexes {
            let price = BigDecimal::from_f32(*ma_prices.get(i).unwrap()).unwrap();
    ...
    if !support_indexes.is_empty() {
        max_support_price = BigDecimal::from_f32(*ma_prices.get(support_indexes[0]).unwrap()).unwrap();
